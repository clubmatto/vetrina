...
// size returns the conversation size in characters.
...
// summarize compresses old messages into one assistant message with a
// dedicated model call.
func summarize(old []Message) Message {
	...
	reply, err := chat([]Message{{
		Role:    "user",
		Content: "Summarize this conversation in ten bullet points:\n\n" + string(b),
	}}, nil)
	if err != nil {
		panic(err)
	}
...
// compact keeps the conversation under budget: once it grows past the limit
// it summarizes the first half with one model call and keeps the recent half.
func compact(messages []Message) []Message {
	total, before := size(messages), len(messages)
	if total < budget {
		return messages
	}
	mid := before / 2
	old, recent := messages[:mid], messages[mid:]
	messages = append([]Message{summarize(old)}, recent...)
	fmt.Printf(">> over budget (%d chars): summarized %d of %d messages into one (%d chars left)\n",
		total, mid, before, size(messages))
	return messages
}
