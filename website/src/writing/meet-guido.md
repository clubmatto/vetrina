---
title: "Meet Guido, your personal race engineer"
description: "We're launching Guido: the spare phone on your sim rig becomes a
  live race dashboard and a voice co-driver for Gran Turismo 7. Local, private,
  zero setup."
date: 2026-09-11
draft: true
tags:
  - guido
  - launch
  - sim-racing
image: /assets/writing/meet-guido.png
image_width: 1200
image_height: 1200
---

Real life race drivers always have someone on the radio. A race engineer
reads the telemetry, does the math mid-race, and tells the driver what matters
when it matters: fuel, tyres, gaps, strategy. The driver drives; the engineer
talks.

Sim racers though just have dashboards. Games like GT7 do stream
their live telemetry, and a healthy ecosystem of second-screen
apps will render it on a phone next to your wheel. Some of these dashboards
are ally cool, they look great, and are quite function most of these numbers
only help if you look at them, at the right moment, and do the math in your
head. Doing this while racing may turn out to be harder than real life racing!

So we built the thing we wanted: not another dashboard, but a
co-driver. Today we're releasing it. Its name is
[Guido](/products/guido/).

At the moment we only support GT7 because we wanted to get this out the door
as quickly as possible. Of course multi sim support is coming very soon, in
the very next release. We chose GT7 as a starting point for two reasons:

- GT7 is underserved compared to other games (to be fair, it's harder to
  make things work on ps4/ps5).
- When we got started with sim racing, GT7 was our first game and we wanted
  to pay hommage to the game that got us started.

## A polished dashboard

Guido turns a spare phone or tablet into a full-screen, landscape race
dashboard: a 16-LED rev bar with shift lights, a gear and speed readout big
enough to parse with peripheral
vision, per-tire temperature gradients, red downshift cues beside the gear
indicator, and the live status of your assists.

We like apps that just work so Guido finds active sims on the
network on its own. Turn on your sim and you're good to go.

Now while this is nice, we do know there are other amazing dashboards out
there (in fact we love them as well!) but that's not the core feature of
Guido. What makes it special is that you can just talk to it and it'll talk
back!

## The engineer in your ear

Guido watches the race the way an engineer would. As the telemetry streams in,
it builds a model of your session: which lap you're on, how much fuel the car
actually burns per lap and how many laps the tank will stretch to. When the math
stops working in your
favor, he speaks up: _"Box this lap."_

It goes both ways, too. Hold the push-to-talk button and ask. How much fuel is
left? How many laps to go? What's the advice? Guido answers out loud, in
plan English, while your hands stay on the wheel. It's also polite
about it: if you just asked about fuel, he won't interrupt you a lap later to
say the thing you already know.

If you're concerned about your data: none of that leaves your Wi-Fi. The speech
recognition runs on your device, the telemetry runs on your network, and there
is no cloud in the middle. Your races belong to you. Unless you're a pro
subscriber, we don't have know your email!

## Free and Pro

The dashboard and push-to-talk questions are free, no account needed.

Signing in with an email unlocks Pro which adds hands-free voice activation (via
"hey Guido!" trigger word) you just talk to Guido instead of reaching for a
button.

Joining the closed beta will grant you Pro access for a year starting from
the official public launch!

## What's next

Guido speaks GT7 today, and it's not staying monolingual for long: Le Mans
Ultimate is the most likely next stop, quite possibly with the Assetto Corsa
family in the same release, and more sims queued up behind those. The
engineer keeps learning too: tyre and shift
intelligence, richer race strategy, more questions it can answer. The
[product page](/products/guido/) stays current as that lands.

The beta is live on Google Play. If you race, give Guido a seat on your rig
and tell us what it should learn next, we read everything at
[hello@matto.club](mailto:hello@matto.club).
